    ...
    public void receive(@Payload List<PaymentRequestAvroModel> messages,
        ...
        messages.forEach(paymentRequestAvroModel -> {
            try {
                ...
                if (psqlException != null && psqlException.getSQLState() != null &&
                        PSQLState.UNIQUE_VIOLATION.getState().equals(psqlException.getSQLState())) {
                    //NO-OP for unique constraint exception
                    log.error("Caught unique constraint exception with SQLState : {}" +
                            " in PaymentRequestKakfaListener for order id: {}",
                            psqlException.getSQLState(), paymentRequestAvroModel.getOrderId());
                } else {
                    throw new PaymentApplicationServiceException("Throwing DataAccessException" +
                            " in PaymentRequestKafkaListener " + e.getMessage(), e);
                }
