...
public class CustomerMessageListenerImpl implements CustomerMessageListener {
    ...
    public void customerCreated(CustomerModel customerModel) {
        ...
        if (customer == null) {
            log.error("Customer couldn't be created in order database with id: {}", customerModel.getId());
            throw new OrderDomainException("Customer couldn't be created in order database with id: " +
                    customerModel.getId());
        }
        log.info("Customer created in order database with id: {}", customer.getId().getValue());
    }
}
